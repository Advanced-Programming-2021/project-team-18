...
public class PreDuelController {
    ...
    public static String requestDuel(String token , String username) {
        HashMap<String,String> headers = new HashMap<>(){{ put("token" , token); put("opponent" , username);}};
        String response = Utility.postRequest(Utility.getSERVER_LOCATION() + REQUEST_DUEL , null , headers);
        return response;
    }
    public static String checkRequest(String token) {
        HashMap<String,String> headers = new HashMap<>(){{ put("token" , token);}};
        String response = Utility.getRequest(Utility.getSERVER_LOCATION() + CHECK_REQUEST , null , headers);
        return response;
    }
    public static String acceptRequest(String token) {
        HashMap<String,String> headers = new HashMap<>(){{ put("token" , token);}};
        String response = Utility.postRequest(Utility.getSERVER_LOCATION() + ACCEPT_REQUEST , null , headers);
        return response;
    }
    public static String receiveLastMessage(String token) {
        HashMap<String,String> headers = new HashMap<>(){{ put("token" , token);}};
        String response = Utility.getRequest(Utility.getSERVER_LOCATION() + RECEIVE_LAST_MESSAGE , null , headers);
        return response;
    }
}
